...
const transferNFTDistributorOwnership = async (
  ...
  const diamond = await ethers.getContractAt(
    'IERC173',
    (await deployments.get('TellerNFTDistributor')).address,
    await ethers.provider.getSigner(deployer)
  )

  const currentOwner = await diamond.owner()
  log('')
  log(`Current NFTDistributor diamond owner: ${currentOwner}`, { indent: 1, star: true })
...
task(
  'transfer-nft-distributor-ownership',
  'Transfer TellerNFTDistributor diamond ownership to a new address'
)
  .addParam('newowner', 'The new owner address', undefined, types.string)
  .setAction(transferNFTDistributorOwnership)
