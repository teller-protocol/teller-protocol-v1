...
const initializeSettings: DeployFunction = async (hre) => {
  ...
  await settingsProxy.initializeProxy(settingsProxy.address, settingsLogic.address)
  const settings = await contracts.get<Settings>('Settings', { from: deployer })
  await settings['initialize(address,address,address)'](logicVersionsRegistryLogic.address, tokens.WETH, tokens.CETH)
