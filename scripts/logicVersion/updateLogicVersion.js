...
module.exports = async (callback) => {
  try {
    ...
    const logicVersionsRegistry = await getContracts.getDeployed(
      teller.logicVersionsRegistry()
    );
    console.log(`Using LogicVersionsRegistry:   ${logicVersionsRegistry.address}`);

    const logicNameBytes32 = toBytes32(web3, logicName);

    const hasLogicVersionResult = await logicVersionsRegistry.hasLogicVersion(
      logicNameBytes32,
      senderTxConfig
    );
    assert(
      hasLogicVersionResult,
      `Logic name ${logicName} / ${logicNameBytes32} is not registered.`
    );

    const getLogicVersionResult = await logicVersionsRegistry.getLogicVersion(
      logicNameBytes32,
      senderTxConfig
    );
    ...
    const newLogicVersionResult = await logicVersionsRegistry.getLogicVersion(
      logicNameBytes32,
      senderTxConfig
    );
