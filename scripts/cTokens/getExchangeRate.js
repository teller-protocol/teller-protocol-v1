...
module.exports = async (callback) => {
  try {
    ...
    console.log(
      `Exchange Rate: ${exchangeRateCurrentResult.toString()} = ${toUnits(
        exchangeRateCurrentResult.toString(),
        exchangeRateDecimals.toString()
      )}`
    );

    const tokenAmountInCToken = tokenAmount
      .times(toDecimals(1, diffDecimals))
      .div(new BigNumber(exchangeRateCurrentResult.toString()));
