...
module.exports = async (callback) => {
  try {
    ...
    if (!loanInfoPrinter.isActive()) {
      callback(
        `Loan ID ${lastLoanID} for borrower ${senderTxConfig.from} is not active. Status: ${loanInfoPrinter.loanInfo.status}`
      );
    }
    ...
    const amountToRepay =
      repayAmountWithDecimals.toString() === '0'
        ? totalOwedWithDecimals
    ...
    const result = await loansInstance.repay(amountToRepay, lastLoanID, senderTxConfig);
