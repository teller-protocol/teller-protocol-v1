...
module.exports = async (callback) => {
  try {
    ...
    if (collateralTokenName !== 'ETH') {
      const collateralTokenInstance = await getContracts.getDeployed(
        tokens.get(collateralTokenName)
      );
      collateralTokenDecimals = await collateralTokenInstance.decimals();
    }
    const collateralAmountWithDecimals = toDecimals(
      collateralAmount,
      collateralTokenDecimals.toString()
    ).toFixed(0);
    ...
    const signedResponse1 = await createSignedLoanTermsResponse(
      web3,
      loanTermsRequest,
      loanResponseInfo1,
      chainId
    ...
    const signedResponse2 = await createSignedLoanTermsResponse(
      web3,
      loanTermsRequest,
      loanResponseInfo2,
      chainId
