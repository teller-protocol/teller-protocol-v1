...
module.exports = async (callback) => {
  try {
    ...
    const loanAmountWithDecimals = toDecimals(
      loanAmount,
      lendingTokenDecimals.toString()
    ).toFixed(0);
    ...
    const loanInfoPrinter = new LoanInfoPrinter(web3, loanInfo, {
      tokenName,
      decimals: lendingTokenDecimals,
    });

    if (!loanInfoPrinter.isTermsSet()) {
      throw new Error(
        `Loan ID ${lastLoanID} for borrower ${senderTxConfig.from} is not TermsSet. Status: ${loanInfoPrinter.loanInfo.status}`
      );
    }

    const maxLoanAmount = loanInfo.loanTerms.maxLoanAmount;
    const amountToBorrow =
      loanAmountWithDecimals === '0' ? maxLoanAmount : loanAmountWithDecimals;
    ...
    console.log(
      `${truffleCommand} --network ${processArgs.network()} --loanID ${lastLoanID} --tokenName ${tokenName} --collTokenName ${collateralTokenName} --senderIndex ${senderIndex} --amount ${amountToBorrow}`
    );
