...
const processArgs = new ProcessArgs(readParams.unpauseLendingPool().argv);
...
module.exports = async (callback) => {
  try {
    ...
    const lendingPool = await getContracts.getDeployed(
    ...
    const currentPaused = await settings.lendingPoolPaused(
      lendingPool.address,
      senderTxConfig
    );
    console.log(`LendingPool Paused (Current)?:     ${currentPaused.toString()}`);

    const result = await settings.unpauseLendingPool(lendingPool.address, senderTxConfig);
    console.log(toTxUrl(result));

    const updatedPaused = await settings.lendingPoolPaused(
      lendingPool.address,
      senderTxConfig
    );
