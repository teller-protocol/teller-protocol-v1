...
module.exports = async (callback) => {
  try {
    ...
    const baseToken =
      baseTokenName.toUpperCase() === 'ETH'
        ? { address: ETH_ADDRESS, decimals: async () => Promise.resolve(18) }
        : await getContracts.getDeployed(tokens.get(baseTokenName));
    const quoteToken =
      quoteTokenName.toUpperCase() === 'ETH'
        ? { address: ETH_ADDRESS, decimals: async () => Promise.resolve(18) }
        : await getContracts.getDeployed(tokens.get(quoteTokenName));
    ...
    const getLatestAnswerResult = await chainlinkAggregator.latestAnswerFor(
      baseToken.address,
      quoteToken.address
    );
    console.log(
      `Lastest Answer:        1 ${baseTokenName} = ${getLatestAnswerResult.toString()} = ${toUnits(
        getLatestAnswerResult.toString(),
        quoteTokenDecimals
      )} ${quoteTokenName}`
    );

    const latestValueResult = await chainlinkAggregator.valueFor(
      baseToken.address,
      quoteToken.address,
      amountWithDecimals
    );
    console.log(
      `Lastest Value:         ${amount} ${baseTokenName} = ${latestValueResult.toString()} = ${toUnits(
        latestValueResult.toString(),
        quoteTokenDecimals
      )} ${quoteTokenName}`
    );
