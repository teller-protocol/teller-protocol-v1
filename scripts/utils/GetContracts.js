...
GetContracts.prototype.getInfo = function ({
  contractName,
  keyName,
  atAddress,
...
GetContracts.prototype.getAddressOrEmpty = function ({
  contractName,
  keyName,
  addressOnProperty = 'address',
}) {
...
GetContracts.prototype.getDeployed = async function ({
  keyName,
  contractName,
  atAddress,
  addressOnProperty = 'address',
  ...
  const { address, name } = this.getInfo({
    keyName,
    contractName,
    atAddress,
    addressOnProperty,
...
GetContracts.prototype.getAllDeployed = async function (
  { teller, tokens },
  tokenName,
  collTokenName
) {
  ...
  const token = await this.getDeployed(tokens.get(tokenName));
  const lendingPool = await this.getDeployed(
    teller.custom(collTokenName).lendingPool(tokenName)
  );
  const loans = await this.getDeployed(teller.custom(collTokenName).loans(tokenName));
  const loanTermsConsensus = await this.getDeployed(
    teller.custom(collTokenName).loanTermsConsensus(tokenName)
  );
