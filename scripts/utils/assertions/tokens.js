 ...
 * Makes sure that the token balance is equal to an amount.
 */
const balanceIs = async ({ token }, { testContext }, { address, expectedBalance }) => {
 ...
 */
const balanceGt = async ({ token }, { testContext }, { address, minBalance }) => {
  const balance = await getBalance(token, address);
  assert(
    balance.gt(minBalance),
    `Expected balance ${balance.toString()} to be greater than ${minBalance}`
  );
};
 ...
 */
const balanceLt = async ({ token }, { testContext }, { address, maxBalance }) => {
  const balance = await getBalance(token, address);
  assert(
    balance.lt(maxBalance),
    `Expected balance ${balance.toString()} to be less than ${maxBalance}`
  );
};
