...
const assertLoanValues = async (
  ...
  if (status != null) {
    assert.strictEqual(
      loanInfo.status.toString(),
      status.toString(),
      'Unexpected loan status'
    );
  }
  if (collateral != null) {
    assert.strictEqual(
      ...
      collateral.toString(),
      'Unexpected loan collateral'
    );
  }
  if (principalOwed != null) {
    assert.strictEqual(
      loanInfo.principalOwed.toString(),
      principalOwed.toString(),
      'Unexpected loan principalOwed'
    );
  }
  if (interestOwed != null) {
    assert.strictEqual(
      loanInfo.interestOwed.toString(),
      interestOwed.toString(),
      'Unexpected loan interestOwed'
    );
  }
  if (liquidated != null) {
  ...
  if (loanInfo.escrow !== NULL_ADDRESS && escrowLoanValue != null) {
    ...
    assert.strictEqual(
      loanValue.toString(),
      escrowLoanValue.toString(),
      'Unexpected loan escrow valueInToken'
    );
  }
};
