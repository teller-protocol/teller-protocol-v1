...
const updateGeneralSetting = async (
  ...
  console.log(
    `Updating general setting ${settingName} from value ${oldValue} to value ${settingValue}`
  );
...
const removeGeneralSetting = async (
  ...
  const removalResult = await atmGovernance.removeGeneralSetting(settingName, txConfig);
  atmGovernanceEvents
...
const addAssetMarketSetting = async (
  ...
  console.log(
    `Adding market setting ${settingName} for asset ${asset} with value ${settingValue}`
  );
...
const updateAssetMarketSetting = async (
  ...
  const oldValue = await atmGovernance.getAssetMarketSetting(asset, settingName);
  console.log(
    `Updating market setting ${settingName} for asset ${asset} from value ${oldValue} with value ${settingValue}`
  );
...
const removeAssetMarketSetting = async (
  ...
  const settingValue = await atmGovernance.getAssetMarketSetting(asset, settingName);
...
const updateDataProvider = async (
  ...
  const oldProvider = await atmGovernance.getDataProvider(dataTypeIndex, providerIndex);
  console.log(
    `Updating data provider ${oldProvider} at index ${providerIndex} with new provider ${newProvider}`
  );
...
const setCRA = async ({ atmGovernance }, { txConfig, testContext }, { _cra }) => {
  console.log(`Setting CRA to commit hash ${_cra}`);
  const result = await atmGovernance.setCRA(_cra);
  atmGovernanceEvents.CRASet(result).emitted(txConfig.from, _cra);
};
