...
module.exports = async (callback) => {
    try {
        ...
        const minAmountWithDecimals = toDecimals(minAmount, tokenDecimals.toString()).toFixed(0);
        ...
        console.log(`Min Amount:                ${minAmountWithDecimals.toString()} = ${minAmount.toString()} ${tokenName}`);

        const requireMoreBalance = BigNumber(accountBalanceWithDecimals.toString()).gte(minAmountWithDecimals);
        assert(requireMoreBalance, `Account ${account} require more ${tokenName} balance. Min amount: ${minAmount} ${tokenName}. Current Balance: ${accountBalance} ${tokenName}`);
