...
module.exports = async (initConfig, { accounts, getContracts }) => {
  ...
  for (const tokenName of tokenNames) {
    const ethLoanTermsConsensusInstance = await getContracts.getDeployed(teller.eth().loanTermsConsensus(tokenName));
    await internalAddSigners(senderTxConfig, signerAddresses, 'ETH', tokenName, ethLoanTermsConsensusInstance );

    const linkLoanTermsConsensusInstance = await getContracts.getDeployed(teller.link().loanTermsConsensus(tokenName));
