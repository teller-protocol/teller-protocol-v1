...
const InitializeableDynamicProxy = artifacts.require(
  './base/InitializeableDynamicProxy.sol'
)
...
const LogicVersionsRegistry = artifacts.require(
  './base/LogicVersionsRegistry.sol'
)
const Escrow = artifacts.require('./base/Escrow.sol')
const EtherCollateralLoans = artifacts.require(
  './base/EtherCollateralLoans.sol'
)
const TokenCollateralLoans = artifacts.require(
  './base/TokenCollateralLoans.sol'
)
...
// Libraries
const LoanLib = artifacts.require('./utils/LoanStorage.sol')
...
module.exports = async function (deployer, network, accounts) {
  ...
  console.log(
    `Deployer account index is ${deployerAccountIndex} => ${deployerAccount}`
  )
  const { maxGasLimit, tokens, atms, compound } = networkConfig
  assert(
    compound.CETH,
    `Compound CETH address not found for network ${network}.`
  )
  assert(maxGasLimit, `Max gas limit for network ${network} is undefined.`)

  // Validations
  tokensRequired.forEach((tokenName) =>
    assert(tokens[tokenName], `${tokenName} token address is not defined.`)
  )

  const txConfig = { gas: maxGasLimit, from: deployerAccount }

  // Creating DeployerApp helper.
  const deployerApp = new DeployerApp(
    deployer,
    web3,
    deployerAccount,
    { InitializeableDynamicProxy, Mock },
  ...
  try {
    ...
    const deployedLogicContractsMap = await deployLogicContracts(contracts, {
      deployerApp,
      txConfig,
      web3,
    })
    ...
    async function deployInitializableDynamicProxy(name) {
      ...
      const proxy = await deployerApp.deployInitializeableDynamicProxy(
        info,
        txConfig
      )
    ...
    const settingsLogic = await deployerApp.deployWith(
      'Settings',
      Settings,
      'teller',
      txConfig
    )
    const settingsProxy = await deployerApp.deployWith(
      'Settings_Proxy',
      UpgradeableProxy,
      'teller',
      txConfig
    ...
    const escrowFactoryInstance = await deployInitializableDynamicProxy(
      logicNames.EscrowFactory
    )
    const chainlinkAggregatorInstance = await deployInitializableDynamicProxy(
      logicNames.ChainlinkAggregator
    )
    ...
    const marketFactoryInstance = await deployInitializableDynamicProxy(
      logicNames.MarketFactory
    )

    console.log(`Deploying LogicVersionsRegistry...`)
    const logicVersionsRegistryLogic = await deployerApp.deployWith(
      'LogicVersionsRegistry',
      LogicVersionsRegistry,
      'teller',
      txConfig
    )
    const logicVersionsRegistryProxy = await deployerApp.deployWith(
      'LogicVersionsRegistry_Proxy',
      UpgradeableProxy,
      'teller',
      txConfig
    )
    ...
    const logicVersionsRegistryInstance = await LogicVersionsRegistry.at(
      logicVersionsRegistryProxy.address
    )
    await logicVersionsRegistryInstance.initialize(settingsInstance.address)
    console.log(
      `LogicVersionsRegistry logic: ${logicVersionsRegistryLogic.address}`
    )
    console.log(
      `LogicVersionsRegistry_Proxy: ${logicVersionsRegistryProxy.address}`
    )
    ...
    async function initializeProxy(name, instance) {
      ...
      assert(
        logicContractInfo.nameBytes32,
        `Name bytes32 is undefined for logic name ${name}.`
      )
    ...
    async function deployDapp(name, unsecured) {
      const info = deployedLogicContractsMap.get(name)
      assert(info, `Deployed logic info is undefined for logic name ${name}.`)
      const proxy = await deployerApp.deployWith(
        `${info.name}_Proxy`,
        DynamicProxy,
        'teller',
        settingsInstance.address,
    ...
    console.log(
      `${'='.repeat(25)} Deployment process finished. ${'='.repeat(25)}`
    )
  } catch (error) {
    console.log(
      '\x1b[33m\x1b[41m\x1b[5m',
    ...
    deployerApp.writeJson()
    console.log(
      '\x1b[33m\x1b[41m\x1b[5m%s\x1b[0m',
      `${'='.repeat(25)} Deployment process FAILED. ${'='.repeat(25)}`
    )
  }
}
