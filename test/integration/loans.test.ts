...
import { createLoan, LoanType } from '../fixtures'
  ...
  function testLoans(market: Market): void {
    ...
    describe.only('createLoanWithTerms, takeOutLoan', () => {
      ...
      it('should be able to take out a loan with collateral', async function () {
        const loanAmount = toBN(100, await lendingToken.decimals())
        ...
        const {
          neededInCollateralTokens: collateralNeeded,
        ...
        const loanInfo = await diamond.getLoan(loanID)

        // Take out loan
        await diamond
          .connect(borrower)
          .takeOutLoan(loanID, loanAmount)
          .should.emit(diamond, 'LoanTakenOut')
          .withArgs(loanID, borrowerAddress, loanAmount)
      })
