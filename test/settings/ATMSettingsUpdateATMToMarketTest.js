...
contract('ATMSettingsUpdateATMToMarketTest', function (accounts) {
  ...
  const newAtM = (borrowedTokenIndex, collateralTokenIndex, atmAddressIndex) => ({
    borrowedTokenIndex,
  ...
  withData(
    {
      ...
      _3_borrowed_token_not_contract: [
        [newAtM(3, 1, 0)],
        newAtM(99, 2, 3),
        0,
        true,
        true,
        false,
        'BORROWED_TOKEN_MUST_BE_CONTRACT',
        true,
      ],
      _4_collateral_token_not_contract: [
        [newAtM(3, 1, 0)],
        newAtM(1, 99, 3),
        0,
        true,
    ...
    function (
      ...
      it(
        t(
          'user',
          'updateATMToMarket',
          'Should (or not) be able to update an ATM from a market.',
          mustFail
        ),
        async function () {
          // Setup
          ...
          const atmAddress =
            atmToMarket.atmAddressIndex === -1
              ? NULL_ADDRESS
              : mocks[atmToMarket.atmAddressIndex];
          ...
          const borrowedToken =
            atmToMarket.borrowedTokenIndex === 99
              ? accounts[0]
              : mocks[atmToMarket.borrowedTokenIndex];
          const collateralToken =
            atmToMarket.collateralTokenIndex === 99
              ? accounts[1]
              : mocks[atmToMarket.collateralTokenIndex];
          ...
          try {
            ...
            const oldAtmToMarket = previousATMToMarkets.find(
              (atm) =>
                atm.borrowedTokenIndex === atmToMarket.borrowedTokenIndex &&
            ...
            atmSettings
              ...
              .emitted(borrowedToken, collateralToken, oldAtmAddress, atmAddress, sender);
          } catch (error) {
            // Assertions
            assert(mustFail);
            assert(error);
