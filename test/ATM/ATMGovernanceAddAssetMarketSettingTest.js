...
contract('ATMGovernanceAddAssetMarketSettingTest', function (accounts) {
    ...
    withData({
        ...
        it(t('user', 'addAssetMarketSetting', 'Should (or not) be able to add an asset market setting.', mustFail), async function() {
            ...
            try {
                ...
                // Invocation
                const result = await instance.addAssetMarketSetting(assetAddress, settingName, settingValue, { from: sender });
    ...
    withData({
        ...
        it(t('user', 'addAssetMarketSetting#2', 'Should not be able to add an asset market setting twice.', mustFail), async function() {
            ...
            try {
                // Invocation
                const result1 = await instance.addAssetMarketSetting(assetAddress, settingName, settingValue, { from: sender });
                assert(result1);
                const result2 = await instance.addAssetMarketSetting(assetAddress, settingName, settingValue, { from: sender });
                assert(result2);
   ...
   // Testing address type
    withData({
        ...
        it(t('user', 'addAssetMarketSetting#3', 'Should not be able to add an asset market setting to a non-contract address.', mustFail), async function() {
            ...
            try {
                // Invocation
                const result = await instance.addAssetMarketSetting(asset, settingName, settingValue, { from: sender });
                assert(result);
