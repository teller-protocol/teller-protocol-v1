  ...
  withData(
    {
      ...
      _2_claim_vested_before_deadline_after_cliff: [
        daoMember2,
        1000,
        3000,
        7000,
    ...
    function (
      ...
      it(
        t(
          'user',
          'withdrawVested',
          'Should or should not be able to claim correctly',
          mustFail
        ),
        async function () {
          // Setup
          await instance.mintVesting(daoMember2, amount, cliff, vestingPeriod, {
            from: daoAgent,
          });
          ...
          try {
            ...
            assert(result);
            tlrToken.vestingClaimed(result).emitted(receipent, amount);
          } catch (error) {
            // Assertions
            assert(mustFail);
            assert(error);
