  ...
  withData(
    ...
    function (senderIndex, settingName, newValue, expectedErrorMessage, mustFail) {
      it(
        t(
          'user',
          'updateAssetMarketSetting#1',
          'Should (or not) be able to update an asset market setting.',
          mustFail
        ),
        async function () {
          // Setup
          ...
          // Precondition
          await instance.addAssetMarketSetting(
            assetAddress,
            settingName,
            SETTING_OLD_VALUE,
            { from: validSender }
          ...
          try {
            // Invocation
            const result = await instance.updateAssetMarketSetting(
              assetAddress,
              settingName,
              newValue,
              { from: sender }
