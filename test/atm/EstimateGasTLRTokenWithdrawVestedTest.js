  ...
  withData(
    {
      ...
      _11_max_vestings_per_wallet_limit: [
        daoMember,
        10,
        2500,
        7000,
        7001,
        MAX_VESTINGS_PER_WALLET_TELLER_CONFIG,
      ],
      _12_max_vestings_gas_limit: [
        daoMember,
        10,
        2500,
        7000,
    ...
    function (recipient, amount, cliff, vestingPeriod, claimTime, vestingsMinted) {
      it(
        t('user', 'withdrawVested', 'Should be able to withdraw using enough gas'),
        async function () {
          // Setup
          // Simulating several vestings minted for wallet to calculate gas change.
          for (let i = 0; i < vestingsMinted; i++) {
            await instance.mintVesting(daoMember, amount, cliff, vestingPeriod, {
              from: daoAgent,
            });
          }
