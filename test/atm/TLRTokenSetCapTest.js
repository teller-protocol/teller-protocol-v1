...
contract('TLRTokenSetCapTest', function (accounts) {
  ...
  withData(
    ...
    function (isOwner, newCap, sender, expectedErrorMessage, mustFail) {
      it(
        t(
          'agent',
          'setCap',
          'Should or should not be able to set cap correctly',
          mustFail
        ),
        async function () {
          if (!isOwner) {
          ...
          try {
            ...
            assert.equal(cap, newCap, 'New supply cap not set!');
            tlrToken.newCap(result).emitted(newCap);
            assert(result);
          } catch (error) {
            // Assertions
            assert(mustFail);
