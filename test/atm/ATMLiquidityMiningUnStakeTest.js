...
contract('ATMLiquidityMiningUnStakeTest', function (accounts) {
  ...
  beforeEach('Setup for each test', async () => {
    ...
    await instance.initialize(settingsInstance.address, governance.address, tlr.address, {
      from: owner,
    });
  });
  ...
  withData(
    ...
    function (amount, offset, isPaused, mustFail, expectedErrorMessage) {
      it(
        t('user', 'unStake#1', 'Should be able or not to unStake tTokens.', mustFail),
        async function () {
          // Setup
          ...
          try {
            // Invocation
            const result = await instance.unStake(tToken.address, amount + offset, {
              from: user,
            });
            // Assertions
            ...
            assert(
              ...
              'user tTokens not received on unStake.'
            );
            const liquidityBalanceAfter = await tToken.balanceOf(instance.address);
            assert(
              parseInt(liquidityBalanceAfter) < parseInt(liquidityBalanceBefore),
              'liquidity tTokens not sent on unStake.'
            );
            assert(
              parseInt(liquidityBalanceAfter) == parseInt(userBalanceBefore),
              'tTokens sent amount is incorrect.'
            );
            const accruedTLRBalance = parseInt(
              await instance.getTLRTotalBalance.call(tToken.address, { from: user })
            );
  ...
  withData(
    ...
    function (amounts, mustFail, expectedErrorMessage) {
      it(
        t(
          'user',
          'unStake#2-one-reward-multiple-unStakes',
          'Should be able or not to unStake tTokens.',
          mustFail
        ),
        async function () {
          // Setup
          ...
          try {
            ...
            for (let i = 0; i < amounts.length; i++) {
              ...
              await helper.advanceBlocks(10);
              // Invocation
              const result = await instance.unStake(tToken.address, amounts[i], {
                from: user,
              });
              // Assertions
              assert(!mustFail, 'It should have failed because data is invalid.');
              ...
              assert.equal(
                userBalanceAfter,
                totalAmount,
                "user didn't receive corresponding tTokens"
              );
              const liquidityBalanceAfter = parseInt(
                await tToken.balanceOf(instance.address)
              );
              assert.equal(
                liquidityBalanceBefore,
                liquidityBalanceAfter + userBalanceAfter,
                'tTokens not sent from liquidity to user'
              );
              // Validating events were emitted
              const accruedTLRBalance = parseInt(
                await instance.getTLRTotalBalance.call(tToken.address, { from: user })
              ...
              liquidityMining
                ...
                .emitted(
                  ...
                  amounts[i],
                  result.receipt.blockNumber,
                  liquidityBalanceBefore - totalAmount,
                  accruedTLRBalance
                );
            }
          } catch (error) {
  ...
  withData(
    ...
    function (amounts, rewards, mustFail, expectedErrorMessage) {
      it(
        t(
          'user',
          'unStake#3-multiple-rewards-multiple-unStakes',
          'Should be able or not to stake tTokens.',
          mustFail
        ),
        async function () {
          // Setup
          ...
          try {
            ...
            const result = await instance.unStake(tToken.address, amounts[1], {
              from: user,
            });
            // Assertions
            assert(!mustFail, 'It should have failed because data is invalid.');
            ...
            assert.equal(
              userBalanceBefore,
              0,
              'user tTokens should have been sent to liquidity contract'
            );
            assert.equal(
              liquidityBalanceAfter,
              0,
              'liquidity tToken balance not correct'
            );
            const userBalanceAfter = parseInt(await tToken.balanceOf(user));
            assert.equal(
              userBalanceAfter,
              totalAmounts,
              'user tToken balance not correct'
            );
            // Validating events were emitted
            const accruedTLRBalance = parseInt(
              await instance.getTLRTotalBalance.call(tToken.address, { from: user })
