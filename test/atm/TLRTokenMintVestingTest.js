  ...
  withData(
    {
      ...
      _3_mint_vesting_zero_address: [
        ...
        60000,
        false,
        'MINT_TO_ZERO_ADDRESS_NOT_ALLOWED',
        true,
      ],
      _4_mint_vesting_above_allowed_max_vesting: [
        true,
        daoAgent,
        daoMember2,
        1000,
      ...
      _5_mint_vesting_invalid_sender: [
        false,
        daoMember2,
        daoMember2,
        1000,
    ...
    function (
      ...
      it(
        t(
          'agent',
          'mintVesting',
          'Should or should not be able to mint correctly',
          mustFail
        ),
        async function () {
          ...
          try {
            // Invocation
            let result = await instance.mintVesting(
              receipent,
              amount,
              cliff,
              vestingPeriod,
            ...
            if (multipleVesting) {
              result = await instance.mintVesting(
                receipent,
                amount,
                cliff,
                vestingPeriod,
            ...
            tlrToken.newVesting(result).emitted(receipent, amount, vestingPeriod);
            // Assertions
            assert(
              !mustFail,
              'It should have failed because the amount is greater than the cap'
            );
          } catch (error) {
            // Assertions
            assert(mustFail);
