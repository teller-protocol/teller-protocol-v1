  ...
  withData(
    ...
    function (senderIndex, settingName, expectedErrorMessage, mustFail) {
      it(
        t(
          'user',
          'removeAssetMarketSetting#1',
          'Should (or not) be able to remove an asset market setting.',
          mustFail
        ),
        async function () {
          // Setup
          ...
          try {
            // Invocation
            const result = await instance.removeAssetMarketSetting(
              assetAddress,
              settingName,
              {
                from: sender,
