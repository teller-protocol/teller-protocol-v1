    ...
    function (
      ...
      it(
        t(
          'user',
          'withdrawTLR',
          'Should be able or not to withdraw TLR tokens.',
          mustFail
        ),
        async function () {
          // Setup
          ...
          try {
            ...
            const result = await instance.withdrawTLR(tToken.address, withdrawAmount, {
              from: aUser,
            });
            // Assertions
            ...
            assert(
              parseInt(userTLRBalanceAfter) > parseInt(userTLRBalanceBefore),
              'user TLR not minted.'
            );
            assert.equal(
              userTLRBalanceAfter,
              withdrawAmount,
              'TLR balance is not correct.'
            );
            // Validating events were emitted
            const remainingAccruedTLR = parseInt(
