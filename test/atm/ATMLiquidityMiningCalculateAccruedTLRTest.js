...
contract('ATMLiquidityMiningCalculateAccruedTLRTest', function (accounts) {
  ...
  beforeEach('Setup for each test', async () => {
    ...
    await instance.initialize(settings.address, governance.address, tlr.address, {
      from: owner,
    });
  });
    ...
    function (tlrRewards, expectedResult, mustFail, expectedErrorMessage) {
      it(
        t(
          'user',
          'calculateAccruedTLR',
          'Should be able or not to calculate accrued TLR tokens.',
          mustFail
        ),
        async function () {
          // Setup
          ...
          try {
            ...
            const result = parseInt(
              await instance.getTLRFloatingBalance.call(tToken.address, { from: user })
            );
            // Assertions
  ...
  withData(
    ...
    function (amounts, expectedResults, mustFail, expectedErrorMessage) {
      it(
        t(
          'user',
          'calculateAccruedTLR#2-one-reward-multiple-stakes',
  ...
  withData(
    {
      _1_multi_reward_multi_stake: [
        [10, 100],
        [10, 100, 1000],
        [100, 10100, 1010100, 1020100],
        false,
        undefined,
      ],
    },
    function (amounts, rewards, expectedResults, mustFail, expectedErrorMessage) {
      it(
        t(
          'user',
          'calculateAccruedTLR#3-multiple-rewards-multiple-stakes',
          'Should be able or not to stake tTokens.',
          mustFail
        ),
        async function () {
          ...
          try {
            ...
            for (let r = 0; r < rewards.length; r++) {
              ...
              const accrued = parseInt(
                await instance.getTLRTotalBalance.call(tToken.address, { from: user })
              );
              const floating = parseInt(
                await instance.getTLRFloatingBalance.call(tToken.address, { from: user })
              );
              ...
              assert.equal(
                accrued,
                floating,
                'Both TLR accrued balances should be equal as no other STAKE(), UNSTAKE(), WITHDRAW() operation has been called.'
              );
              assert.equal(
                floating,
                expectedResults[r],
                'Incorrect floating TLR calculation.'
              );
            }
            ...
            assert.equal(
              floating,
              FLOATING_SHOULD_RESET_AFTER_STAKE_OPERATION,
              'Floating should be zero after stake().'
            );
            ...
            assert.equal(
              ...
              'Incorrect accrued TLR calculation.'
            );
          } catch (error) {
            assert(mustFail);
            assert(error);
