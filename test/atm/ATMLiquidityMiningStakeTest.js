    ...
    function (
      ...
      it(
        ...
        async function () {
          ...
          try {
            // Invocation
            const result = await instance.stake(tToken.address, stakeAmount + offset, {
              from: user,
            });
            // Assertions
            ...
            assert(
              ...
              'user tTokens not sent on stake.'
            );
            ...
            assert(
              ...
              'tTokens not received on stake.'
            );
            assert(
              parseInt(liquidityBalanceAfter) == parseInt(userBalanceBefore),
              'tTokens received are less than sent amount.'
            );
  ...
  withData(
    ...
    function (amounts, mustFail, expectedErrorMessage) {
      it(
        t(
          'user',
          'stake#2-one-reward-multiple-stakes',
          'Should be able or not to stake tTokens.',
          mustFail
        ),
        async function () {
          try {
            ...
            for (let i = 0; i < amounts.length; i++) {
              ...
              // Invocation
              const result = await instance.stake(tToken.address, amounts[i], {
                from: user,
              });
              // Assertions
              assert(!mustFail, 'It should have failed because data is invalid.');
  ...
  withData(
    ...
    function (amounts, rewards, mustFail, expectedErrorMessage) {
      it(
        t(
          'user',
          'stake#3-multiple-rewards-multiple-stakes',
          'Should be able or not to stake tTokens.',
          mustFail
        ),
        async function () {
          ...
          try {
            // Invocation
            const result = await instance.stake(tToken.address, amounts[1], {
              from: user,
            });
            // Assertions
            assert(!mustFail, 'It should have failed because data is invalid.');
            ...
            assert.equal(
              ...
              totalAmounts,
              'Liquidity contract tToken balance error'
            );
            const accruedTLRBalance = parseInt(
              await instance.getTLRTotalBalance.call(tToken.address, { from: user })
            );
