...
contract('EscrowFactoryCreateEscrowTest', function (accounts) {
  ...
  beforeEach(async () => {
    ...
    settingsInstance = await createTestSettingsInstance(Settings, {
      ...
      onInitialize: async (
        instance,
        { interestValidator, chainlinkAggregator, atmSettings }
      ) => {
    ...
    await versionsRegistry.createLogicVersion(
      web3.utils.soliditySha3('Escrow'),
      escrowLogic.address
    );
  });
    ...
    function (
      ...
      it(
        t(
          'loans',
          'createEscrow',
          'Should not be able to create an escrow contract.',
          mustFail
        ),
        async function () {
          // Setup
          ...
          try {
            // Invocation
            const result = await escrowFactoryInstance.createEscrow(
              loans.address,
              loanID
            );

            // Assertions
            escrowFactory.escrowCreated(result).emitted(borrower, loans.address, loanID);
