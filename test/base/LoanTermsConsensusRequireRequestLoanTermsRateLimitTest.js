...
contract('LoanTermsConsensusRequireRequestLoanTermsRateLimitTest', function (accounts) {
  ...
  beforeEach('Setup for each test', async () => {
    ...
    await instance.initialize(owner, callerInstance.address, settingsInstance.address);
  });
  ...
  withData(
    ...
    function (
      ...
      it(
        t(
          'user',
          'requireRequestLoanTermsRateLimit',
          'Should validate if request loan terms rate limit is valid or not.',
          mustFail
        ),
        async function () {
          ...
          const currentTimestamp = isFirstTime
            ? 0
            : await timer.getCurrentTimestampInSeconds();
          const lastLoanTermsRequested =
            currentTimestamp - lastLoanTermsRequestedSecondsBack;
          ...
          try {
            // Invocation
            const result = await instance.externalRequireRequestLoanTermsRateLimit(
              request
            );
