...
const InitializeableDynamicProxyMock = artifacts.require(
  './mock/base/InitializeableDynamicProxyMock.sol'
);
    ...
    function (previousLogicName, logicName, expectedErrorMessage, mustFail) {
      it(
        t(
          'user',
          'initializeProxy',
          'Should (or not) be able to initialize the proxy.',
          mustFail
        ),
        async function () {
          // Setup
          ...
          try {
            // Invocation
            const result = await instance.initializeProxy(
              settings.address,
              logicNameBytes32,
              { from: sender }
            );
