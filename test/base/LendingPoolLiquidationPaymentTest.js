  ...
  withData(
    {
      ...
      _2_cTokenSupported_transferFromFail: [
        accounts[1],
        true,
        true,
        false,
      ...
      _3_cTokenSupported_notLoansSender: [
        accounts[1],
        false,
        true,
        true,
      ...
      _6_cTokenNotSupported_transferFromFail: [
        accounts[1],
        true,
        false,
        false,
      ...
      _7_cTokenNotSupported_notLoansSender: [
        accounts[1],
        false,
        false,
        true,
    ...
    function (
      ...
      it(
        t(
          'user',
          'liquidationPayment',
          'Should able (or not) to liquidate payment.',
          mustFail
        ),
        async function () {
          // Setup
          ...
          try {
            // Invocation
            const result = await instance.liquidationPayment(
              amountToLiquidate,
              liquidator,
              { from: sender }
            );
            ...
            assert(result);
            lendingPool.paymentLiquidated(result).emitted(liquidator, amountToLiquidate);
          } catch (error) {
            // Assertions
