  ...
  withData(
    {
      ...
      _4_loan_liquidated_caller_not_loans_instance: [
        loanStatus.Closed,
        accounts[1],
        accounts[2],
        true,
    ...
    function (
      ...
      it(
        t(
          ...
          'Should be able to claim tokens based on the value of collateral.',
          mustFail
        ),
        async function () {
          // Setup
          ...
          try {
            ...
            let result;
            if (isCallerLoans) {
              const loansTx = await loans.externalEscrowClaimTokensByCollateralValue(
                recipient,
                tokenBalance,
                loan.id
              );
              result = await truffleAssertions.createTransactionResult(
                instance,
                loansTx.tx
              );
            } else {
              result = await instance.claimTokensByCollateralValue(
                recipient,
                tokenBalance,
                { from: borrower }
              );
            ...
            escrow.tokensClaimed(result).emitted(recipient);
          } catch (error) {
