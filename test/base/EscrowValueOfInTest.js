...
contract('EscrowValueOfInTest', function (accounts) {
  ...
  before(async () => {
    ...
    await CDAIInstance.givenMethodReturnUint(cTokenEncoder.encodeDecimals(), '18');
  ...
  withData(
    ...
    function ({ isCToken, expectedValue, mustFail, expectedErrorMessage }) {
      it(
        t(
          'escrow',
          '_valueOfIn',
          'Should be able to calculate the value of a base asset in a quote asset.',
          mustFail
        ),
        async function () {
          // Setup
          ...
          try {
            // Invocation
            const value = await instance.externalValueOfIn.call(
              baseToken.address,
              ETH_ADDRESS,
              '1'
            );
