...
contract('TTokenRegistryIsTTokenTest', function (accounts) {
  ...
  beforeEach('Setup for each test', async () => {
    ...
    const settingsInstance = await createTestSettingsInstance(Settings, {
      ...
      onInitialize: async (
        instance,
        { escrowFactory, interestValidator, atmSettings }
      ) => {
  ...
  withData(
    ...
    function (contractAddress, expectedReturn, expectedErrorMessage, mustFail) {
      it(
        t(
          'user',
          'isTToken',
          'Should be able to check if a given TToken contract has been registered.',
          mustFail
        ),
        async function () {
          // Setup
          ...
          const tokenAdd =
            contractAddress === 0 ? tokenContract.address : newTokenContract.address;
