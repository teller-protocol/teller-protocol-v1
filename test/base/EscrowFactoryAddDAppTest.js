    ...
    function (
      ...
      it(
        t(
          'escrowFactory',
          'addDapp',
          'Should be able (or not) to add a new dapp.',
          mustFail
        ),
        async function () {
          // Setup
          for (const previousDappIndex of previousDaapIndexes) {
            await instance.addDapp(getInstance(mocks, previousDappIndex, 1), unsecured, {
              from: owner,
            });
          }
          ...
          try {
            // Invocation
            const result = await instance.addDapp(dappAddress, unsecured, {
              from: caller,
            });
            ...
            escrowFactory.newDappAdded(result).emitted(caller, dappAddress, unsecured);
