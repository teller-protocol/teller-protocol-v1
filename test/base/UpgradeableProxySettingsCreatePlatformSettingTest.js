    ...
    function (caller, admin, mustFail, expectedErrorMessage) {
      it(
        t(
          ...
          'Should be able to use multiple proxies of a contract and all share the same state.',
          mustFail
        ),
        async function () {
          try {
            ...
            await v1.initializeProxy(settings.address, initLogic.address, {
              from: admin,
            });
            ...
            await settingsOfProxy2.createPlatformSetting(settingName, 100, 0, 2000, {
              from: caller,
            });
