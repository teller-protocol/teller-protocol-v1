    ...
    function (
      ...
      it(
        t(
          ...
          `Should (or not) be able to remove a platform setting.`,
          mustFail
        ),
        async function () {
          // Setup
          ...
          try {
            ...
            const previousSetting = previousSettings.find(
              (item) => item.name === platformNameToRemove
            );

            settings
              .platformSettingRemoved(result)
              .emitted(platformNameToRemoveBytes32, sender, previousSetting.value);
