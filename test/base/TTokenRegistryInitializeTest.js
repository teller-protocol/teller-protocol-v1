...
contract('TTokenRegistryInitializeTest', function (accounts) {
  ...
  withData(
    ...
    function (contractAddress, expectedErrorMessage, mustFail) {
      it(
        t(
          'user',
          'initialize',
          'Should (or should not) be able to initialize the TTokenRegistry contract.',
          mustFail
        ),
        async function () {
          // Setup
          ...
          const settingsAdd =
            contractAddress === 0 ? settingsInstance.address : NULL_ADDRESS;
