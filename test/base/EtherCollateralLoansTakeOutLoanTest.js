...
contract('EtherCollateralLoansTakeOutLoanTest', function (accounts) {
  ...
  withData(
    ...
    function (
      ...
      it(
        t('user', 'takeOutLoan#1', 'Should not able to take out a loan.', true),
        async function () {
          // Setup
  ...
  withData(
    ...
    function (amountToBorrow, collateralRatio) {
      it(
        t(
          'user',
          'takeOutLoan#2',
          'Should able to take out an under collateralized loan with an Escrow contract.',
          false
        ),
        async function () {
          // Setup
          ...
          assert.notEqual(
            loan.escrow.toString(),
            NULL_ADDRESS,
            'Escrow contract was not created'
          );
          const count = await createdEscrowMock.invocationCountForMethod.call(
            escrowInterfaceEncoder.encodeInitialize()
          );
          ...
          const createLoanCallData = pool.contract.methods
            .createLoan(amountToBorrow.toString(), loan.escrow.toString())
            .encodeABI();
          const createLoanCallCount = await lendingPoolInstance.invocationCountForCalldata.call(
            createLoanCallData
          );
          assert.equal(
            createLoanCallCount.toString(),
            '1',
            'Lending pool not called to create loan with escrow address'
          );
        }
      );
    }
    ...
    function (amountToBorrow, collateralRatio) {
      it(
        t(
          'user',
          'takeOutLoan#3',
          'Should able to take out an over collateralized loan with to their own wallet.',
          false
        ),
        async function () {
          // Setup
