...
contract('EtherCollateralLoansRepayTest', function (accounts) {
  ...
  withData(
    {
      _1_to_pay_more_than_owed: [
        12345678,
        40291,
        13000000,
        BigNumber('3000000000000000000'),
        undefined,
        false,
      ],
      _2_to_pay_zero: [
        ...
        0,
        BigNumber('3000000000000000000'),
        'AMOUNT_VALUE_REQUIRED',
        true,
      ],
      _3_to_less_than_owed: [
        12345678,
        40291,
        12345677,
        BigNumber('3276312217812723563'),
        undefined,
        false,
      ],
      _4_to_pay_exact_owed: [
        12345678,
        40291,
        12385969,
        BigNumber('3276312217812723563'),
    ...
    function (
      ...
      it(t('user', 'repay', 'Should able to repay your loan.', false), async function () {
        ...
        const loan = createLoan({
          id: mockLoanID,
          loanTerms,
          collateral: loanCollateral.toString(),
        ...
        try {
          ...
          if (newPrincipalOwed + newInterestOwed == 0) {
            ...
            assert.equal(
              totalCollateral.minus(loanCollateral).toFixed(),
              totalAfter.toString(),
              'Total collateral not match'
            );
            assert.equal(
              BigNumber(contractBalBefore).minus(loanCollateral),
              contractBalAfter.toString(),
              'Contract balance not match'
            );
            assert.equal(
              BigNumber(borrowerBalBefore).plus(loanCollateral),
              borrowerBalAfter.toString(),
              'Borrower balance not match'
            );
