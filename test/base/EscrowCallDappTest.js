...
contract('EscrowCallDappTest', function (accounts) {
  ...
  beforeEach(async () => {
    await createTestSettingsInstance(Settings, {
      ...
      Mock,
      initialize: true,
      onInitialize: (instance, { escrowFactory }) => {
        settings = instance;
  ...
  withData(
    {
      ...
      _4_with_invalid_function_signature: [
        5,
        true,
        true,
        true,
    ...
    function (
      ...
      it(
        t(
          'user',
          'callDapp',
          'Should be able (or not) to call a function on a whitelisted dapp contract',
          mustFail
        ),
        async function () {
          // Setup
          ...
          await factory.givenMethodReturn(
            ...
            encodeDappConfigParameter(web3, {
              exists: enableDapp,
              unsecured: unsecuredDapp,
            })
          );
