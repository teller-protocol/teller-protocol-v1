...
contract('BaseEscrowDappTokenUpdatedTest', function (accounts) {
  ...
  withData(
    {
      _1_remove_dai_remove_usdc_add_link: [
        100,
        0,
        -1,
        100,
      ...
      _2_remove_dai_add_usdc_remove_link: [
        100,
        0,
        -1,
        0,
      ...
      _3_add_dai_remove_usdc_remove_link: [
        0,
        100,
        0,
        100,
      ...
      _5_remove_dai_remove_usdc_remove_link: [
        100,
        0,
        -1,
        100,
    ...
    function (
      ...
      it(
        t(
          'dapp',
          '_tokenUpdated',
          'Should be able to update its record of tokens that have (or not) balances.',
          mustFail
        ),
        async function () {
          // Setup
          ...
          try {
            ...
            assert(
              initialDaiIndex.toString() === expectedInitialDaiIndex.toString(),
              'Incorrect initial DAI index'
            );
            ...
            assert(
              initialUsdcIndex.toString() === expectedInitialUsdcIndex.toString(),
              'Incorrect initial USDC index'
            );

            const expectedInitialLinkIndex = initialLink
              ? initialUsdc
                ? initialDai
                  ? 2
                  : 1
            ...
            assert(
              initialLinkIndex.toString() === expectedInitialLinkIndex.toString(),
              'Incorrect initial LINK index'
            );
            ...
            assert(
              daiIndex.toString() === expectedDaiIndex.toString(),
              'DAI was not updated correctly.'
            );

            const usdcIndex = await instance.findTokenIndex.call(usdc.address);
            assert(
              usdcIndex.toString() === expectedUsdcIndex.toString(),
              'USDC was not updated correctly.'
            );

            const linkIndex = await instance.findTokenIndex.call(link.address);
            assert(
              linkIndex.toString() === expectedLinkIndex.toString(),
              'LINK was not updated correctly.'
            );
          } catch (error) {
            assert(error);
