...
contract('LoanTermsConsensusHashesTest', function (accounts) {
  ...
  withData(
    {
      _1_mainnet_first_test_hashRequest: [
        chains.mainnet,
        accounts[2],
        accounts[1],
        234764,
    ...
    function (chainId, borrower, recipient, requestNonce, amount, duration, requestTime) {
      it(
        t(
          'user',
          'hashRequest',
          'Should correctly calculate the hash for a request',
          false
        ),
        async function () {
          ...
          let expectedResult = ethUtil.bufferToHex(
            hashLoanTermsRequest(request, loansInstance.address, chainId)
          );
    ...
    function (
      ...
      it(
        t(
          'user',
          'hashResponse',
          'Should correctly calculate the hash for a response',
          false
        ),
        async function () {
          const response = createUnsignedLoanResponse(
            signer,
            responseTime,
            interestRate,
            collateralRatio,
          ...
          const requestHash = ethUtil.bufferToHex(
            hashLoanTermsRequest(request, accounts[4], chainId)
          );

          const expectedHash = ethUtil.bufferToHex(
            hashLoanTermsResponse(response, requestHash, chainId)
          );
          await instance.mockChainId(chainId);
