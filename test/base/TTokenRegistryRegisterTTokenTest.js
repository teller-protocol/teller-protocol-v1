...
contract('TTokenRegistryRegisterTTokenTest', function (accounts) {
  ...
  withData(
    ...
    function (senderAdd, contractAddress, expectedErrorMessage, mustFail) {
      it(
        t(
          'user',
          'registerTToken',
          'Should (or should not) be able to register a TToken contract.',
          mustFail
        ),
        async function () {
          // Setup
          ...
          try {
            ...
            assert(result);
            tTokenRegistry.tTokenRegistered(result).emitted(tokenAdd, sender);
          } catch (error) {
            assert(mustFail);
            assert(error);
