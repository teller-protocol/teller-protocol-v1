...
contract('ERC20TransferTest', function (accounts) {
  ...
  withData(
    ...
    function (sender, amount, receiver) {
      it(
        t('user', 'transfer', 'Should be able transfer tokens.', false),
        async function () {
          // Setup
          ...
          // Invocation
          const result = await instance.transfer(receiver, amount.toString(), {
            from: sender,
          });

          // Assertions
          assert(result);
          erc20.transfer(result).emitted(sender, receiver, amount);

          const senderBalanceFinal = await instance.balanceOf(sender);
          const senderBalanceDiff = BigNumber(senderBalanceFinal.toString()).minus(
            BigNumber(senderBalanceInitial.toString())
          );
          const receiverBalanceFinal = await instance.balanceOf(receiver);
          const receiverBalanceDiff = BigNumber(receiverBalanceFinal.toString()).minus(
            BigNumber(receiverBalanceInitial.toString())
          );
  ...
  withData(
    ...
    function (sender, amount, receiver) {
      it(
        t('user', 'transfer', 'Should NOT be able transfer tokens.', true),
        async function () {
          // Setup
