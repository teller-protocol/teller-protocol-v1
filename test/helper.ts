...
export class SettingsTestHelper {
  ...
  public async updateCTokenAddressSetting(assetSettingsAddress: string, newCTokenAddress: string, senderAddress = this.addresses[0]) {
    if (!this.contracts.settings) throw 'No settings contract deployed';
    const assetSettings = await load<AssetSettings>("AssetSettings", await this.contracts.settings.assetSettings());
...
export class LoansTestHelper {
  ...
  public async createLoanWithTerms(request: Parameters<LoansBase["createLoanWithTerms"]>[0], responses: Parameters<LoansBase["createLoanWithTerms"]>[1], collateralAmount: number, senderAddress = this.addresses[0]) {
  ...
  public async depositCollateral(borrowerAddress: string, amountToDeposit: number, loanID: number, senderAddress = this.addresses[0]) {
    if (!this.contracts.loans?.ETH_DAI) throw 'No loans contract deployed';
