...
contract('ChainlinkAggregatorRemoveForTest', function (accounts) {
    ...
    function (
      ...
      it(
        t(
          ...
          'remove',
          'Should be able to (or not) remove support for a pair to Chainlink.',
          mustFail
        ),
        async function () {
          ...
          for (const srcAddress of srcAddresses) {
            for (const dstAddress of dstAddresses) {
              await instance.add(srcAddress, dstAddress, srcAddress, {
                from: accounts[ownerIndex],
              });
            }
          }

          try {
            await instance.methods['remove(address,address)'](
              srcAddresses[0],
              dstAddresses[0],
  ...
  withData(
    ...
    function (senderIndex: string, markets: number, mustFail, expectedErrorMessage) {
      it(
        t(
          'chainlink aggregator',
          'remove',
          'Should be able to (or not) remove support for all markets for a token.',
          mustFail
        ),
        async function () {
          // NOTE: creating 2 source markets means there is 1 source token and multiple destination tokens
          ...
          try {
            await instance.methods['remove(address)'](srcAddress, {
              from: accounts[senderIndex],
            });
