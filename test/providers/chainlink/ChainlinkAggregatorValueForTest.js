...
contract("ChainlinkAggregatorValueForTest", function(accounts) {
  ...
  withData({
    ...
    it(t("loans", "latestAnswerFor", "Should be able (or not) to get the latest answer for a market.", mustFail), async function() {
      try {
        ...
        const expectedLatestAnswer = toDecimals(new BN(borrowedAmount).multipliedBy(chainlinkResponsePrice), collateralTokenDecimals);
  ...
  withData({
    ...
    it(t("loans", "latestAnswerFor", "Should be able (or not) to get the latest answer for a market.", mustFail), async function() {
      try {
        const borrowedToken = await mockERC20({ decimals: borrowedTokenDecimals }, { Mock, encoder: erc20InterfaceEncoder });
        const routeToken = await mockERC20({ decimals: collateralTokenDecimals }, { Mock, encoder: erc20InterfaceEncoder });
        const collateralToken = await mockERC20({ decimals: collateralTokenDecimals }, { Mock, encoder: erc20InterfaceEncoder });
        const chainlinkAggregator = await Mock.new();
        await chainlinkAggregator.givenMethodReturnUint(
          aggregatorV2V3InterfaceEncoder.encodeLatestAnswer(),
        ...
        const expectedLatestAnswer = toDecimals((new BN(borrowedAmount).multipliedBy(chainlinkResponsePrice)).multipliedBy(chainlinkResponsePrice), collateralTokenDecimals);
        assert.equal(valueForResult.toString(), expectedLatestAnswer.toString());
        assert(!mustFail);
      } catch (error) {
        assert(mustFail, error.message);
