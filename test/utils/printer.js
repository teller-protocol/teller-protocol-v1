...
const printInterestRate = ({ loanInfo }, { tokenInfo }) => {
  ...
  const amount =
    loanInfo.status.toString() === loanStatus.Active.toString()
  ...
  console.log(
    `Duration:              ${durationInDays} days = ${loanTerms.duration} secs`
  );

  console.log(
    `APR:                   ${loanTerms.interestRate} == ${BigNumber(
  ...
  console.log(
    `Yearly (% / ${tokenName}):     ${interestRatePerYear
      .times(100)
      .toFixed(8)} % / ${interestPerYear.toFixed(8)} ${tokenName}`
  );

  const interestRatePerMonth = interestRatePerYear.div(12);
  const interestPerMonth = interestPerYear.div(12);
  console.log(
    `Monthly (% / ${tokenName}):    ${interestRatePerMonth
      .times(100)
      .toFixed(8)} % / ${interestPerMonth.toFixed(8)} ${tokenName}`
  );

  const interestRatePerDay = interestRatePerYear.div(YEAR);
  const interestPerDay = interestPerYear.div(YEAR);
  console.log(
    `Daily (% / ${tokenName}):      ${interestRatePerDay.toFixed(
      8
    )} % / ${interestPerDay.toFixed(8)} ${tokenName}`
  );
  ...
  console.log(
    `Duration (% / ${tokenName}):   ${interestRatePerDuration.toFixed(
      8
    )} % / ${interestPerDuration.toFixed(8)} ${tokenName}`
  );
  console.groupEnd();
  console.log(SIMPLE_SEPARATOR);
};
...
const printLoanTerms = ({ loanTerms }, { tokenInfo }) => {
  ...
  console.log(
    `Collateral Ratio:  ${loanTerms.collateralRatio} == ${BigNumber(
  ...
  console.log(
    `Max. Loan Amount:  ${loanTerms.maxLoanAmount} = ${maxLoanAmountUnits} ${tokenInfo.symbol}`
  );
  console.groupEnd();
  console.log(SIMPLE_SEPARATOR);
};

const printEscrow = async (
  { settings, loans },
  { testContext },
  { loanId, tokenInfo }
) => {
  ...
  const collateralBufferSetting = await settings.getPlatformSetting(
    collateralBufferBytes32
  );
  const collateralBuffer = parseInt(collateralBufferSetting.value);

  console.log(
    `Collateral Buffer (%):             ${collateralBuffer} = ${collateralBuffer / 100} %`
  );
  console.log(`Escrow Address:                    ${escrow.address}`);
  console.log(
    `Loan Value (borrowed token):      ${loanValue} = ${toUnits(
      loanValue,
      tokenInfo.decimals
    ).toFixed(6)} ${tokenInfo.symbol}`
  );
  console.groupEnd();
  console.log(SIMPLE_SEPARATOR);
};
...
const printTokensInfo = ({ tokenInfo, collateralTokenInfo }) => {
  ...
  console.log(
    `Name / Symbol: ${collateralTokenInfo.name} / ${collateralTokenInfo.symbol}`
  );
...
const printPairAggregator = async (
  { chainlinkAggregator },
  ...
  const latestAnswer = await chainlinkAggregator.latestAnswerFor(
    tokenInfo.address,
  ...
  console.log(
    `Price:             1 ${tokenInfo.symbol} = ${lastestAnswerUnits.toFixed(
      INFO_DECIMALS
    )}  ${collateralTokenInfo.symbol}`
  );
  console.log(
    `Price:             1 ${collateralTokenInfo.symbol} = ${inversePrice.toFixed(
      INFO_DECIMALS
    )}  ${tokenInfo.symbol}`
  );
...
const printCollateralInfo = (
  { loanInfo, collateralInfo },
  { tokenInfo, collateralTokenInfo }
) => {
  const tokenName = tokenInfo.symbol;
  const tokenDecimals = tokenInfo.decimals;
  ...
  const collRequiredCollTokensUnits = toUnits(
  ...
  const collRequiredTokensUnits = toUnits(
    collateralInfo.neededInLendingTokens,
    tokenDecimals
  );
  ...
  console.log(
    `Coll. Required (in ${tokenName}):      ${collRequiredTokensUnits.toFixed(
      INFO_DECIMALS
    )} ${tokenName} (${collateralRatio}% coll. ratio)`
  );
  console.log(
    `Coll. Required (in ${collateralTokenName}):       ${collRequiredCollTokensUnits.toFixed(
      INFO_DECIMALS
    )} ${collateralTokenName}`
  );
  console.log(
    `Current Coll. (in ${collateralTokenName}):        ${currentCollUnits.toFixed(
      INFO_DECIMALS
    )} ${collateralTokenName}`
  );
...
const printLoan = ({ loanInfo, totalOwed }, { tokenInfo, collateralTokenInfo }) => {
  ...
  console.log(
    `Borrowed Amount:       ${loanInfo.borrowedAmount} = ${toUnits(
      loanInfo.borrowedAmount,
      tokenDecimals
    )} ${tokenName}`
  );
  console.log(
    `Terms Expiry:          ${loanInfo.termsExpiry} secs = ${secondsToDays(
      loanInfo.termsExpiry
    ).toFixed(2)} days`
  );
  if (loanInfo.status.toString() === loanStatus.TermsSet.toString()) {
    ...
    console.log(
      `Start Time:            ${loanInfo.loanStartTime} / ${new Date(
        parseInt(loanInfo.loanStartTime) * 1000
      )}`
    );
    const loanEndTime =
      parseInt(loanInfo.loanStartTime) + parseInt(loanInfo.loanTerms.duration);
    console.log(
      `End Time:              ${loanEndTime} / ${new Date(parseInt(loanEndTime) * 1000)}`
    );
  }
  console.log(
    `Collateral:            ${loanInfo.collateral} = ${toUnits(
      loanInfo.collateral,
      collateralTokenDecimals
    )} ${collateralTokenName}`
  );

  const lastCollateralInDate =
    loanInfo.lastCollateralIn === '0'
      ? ''
      : new Date(parseInt(loanInfo.lastCollateralIn) * 1000);
  console.log(
    `Last Collateral In:    ${loanInfo.lastCollateralIn} / ${lastCollateralInDate}`
  );
  console.log(
    `Principal Owed:        ${loanInfo.principalOwed} = ${toUnits(
      loanInfo.principalOwed,
      tokenDecimals
    )} ${tokenName}`
  );
  console.log(
    `Interest Owed:         ${loanInfo.interestOwed} = ${toUnits(
      loanInfo.interestOwed,
      tokenDecimals
    )} ${tokenName}`
  );
