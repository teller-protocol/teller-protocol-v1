...
const { daysToSeconds } = require('./consts');
...
const createSettingsInstance = async (
  ...
  if (shouldInitPlatformSettings) {
    await initPlatformSettings(instance, { ...platformSettings, txConfig }, {});
  }
...
module.exports = {
  ...
  createTestSettingsInstance: async (
    Settings,
    { from, Mock, initialize, onInitialize },
    platformSettings
  ) => {
    ...
    return await createSettingsInstance(
      Settings,
      { txConfig: { from }, Mock, initialize, onInitialize },
      platformSettings
    );
  },
  printPlatformSetting: (
    { value, min, max, exists },
    { settingName, settingNameBytes32 }
  ) => {
