...
class LoanInfoPrinter {
  constructor(web3, loanInfo, token, collateralToken) {
    ...
    this.collateralToken = {
      tokenName: collateralToken.tokenName,
...
LoanInfoPrinter.prototype.getTotalOwed = function () {
  return BigNumber(this.loanInfo.principalOwed).plus(
    BigNumber(this.loanInfo.interestOwed)
  );
};
...
LoanInfoPrinter.prototype.getCollateralInTokens = function (price) {
  return this.getCollateral().times(this.getAWholeToken()).div(price);
};

LoanInfoPrinter.prototype.getTotalTokensPaymentInLiquidation = function (
  price,
  ethPricePercentage
) {
...
LoanInfoPrinter.prototype.isLiquidable = async function (price) {
  return (
    this.isActive() &&
    (this.isCollateralNeededGtCollateral(price) || (await this.isEndTimeLtNow()))
  );
};
