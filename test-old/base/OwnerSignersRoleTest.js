...
contract('OwnerSignersRoleTest', function (accounts) {
    ...
    }, function(previousSignerIndexes, senderIndex, newSignerIndex, expectedErrorMessage, mustFail) {
        it(t('user', 'addSigner', 'Should be able (or not) to add a new signer.', mustFail), async function() {
            // Setup
            ...
            const newSignerAddress = newSignerIndex === -1 ? NULL_ADDRESS: accounts[newSignerIndex];
    ...
    }, function(previousSignerIndexes, senderIndex, signerIndexToRemove, expectedErrorMessage, mustFail) {
        it(t('user', 'removeSigner', 'Should be able (or not) to remove a new signer.', mustFail), async function() {
            // Setup
            for (const previousSignerIndex of previousSignerIndexes) {
                const previusSignerAddress = accounts[previousSignerIndex];
                await instance.addSigner(previusSignerAddress, { from: owner });
            }
            const signerAddressToRemove = signerIndexToRemove === -1 ? NULL_ADDRESS: accounts[signerIndexToRemove];
            const sender = accounts[senderIndex];

            // Invocation
            try {
                const result = await instance.removeSigner(signerAddressToRemove, { from: sender });
    ...
    withData({
        ...
        it(t('user', 'isSigner', 'Should be able (or not) to remove a new signer.', false), async function() {
            // Setup
            for (const previousSignerIndex of previousSignerIndexes) {
                const previusSignerAddress = accounts[previousSignerIndex];
                await instance.addSigner(previusSignerAddress, { from: owner });
            }
            const addressToTest = signerIndexToTest === -1 ? NULL_ADDRESS: accounts[signerIndexToTest];
