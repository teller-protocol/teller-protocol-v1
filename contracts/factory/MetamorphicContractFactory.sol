...
contract MetamorphicContractFactory {
    ...
    bytes private immutable _metamorphicContractInitializationCode;

    // store hash of the initialization code for metamorphic contracts as well.
    bytes32 private immutable _metamorphicContractInitializationCodeHash;

    // store init code for transient contracts that deploy metamorphic contracts.
    bytes private immutable _transientContractInitializationCode;

    // store the hash of the initialization code for transient contracts as well.
    bytes32 private immutable _transientContractInitializationCodeHash;
        ...
        returns (address metamorphicContractAddress)
    {
        ...
        if (data.length > 0 || msg.value > 0) {
            ...
            (bool success, ) =
                deployedMetamorphicContract.call{ value: msg.value }(data);
        ...
        returns (address metamorphicContractAddress)
    {
        ...
        if (data.length > 0 || msg.value > 0) {
            /* solhint-disable avoid-call-value */
            (bool success, ) =
                metamorphicContractAddress.call{ value: msg.value }(data);
            /* solhint-enable avoid-call-value */
     ...
     * transient contract.
     * @return transientContractAddress Address of the corresponding transient contract.
     */
     ...
     */
    function _getMetamorphicContractAddress(bytes32 salt)
        internal
        view
        returns (address)
    {
     ...
     * given a particular salt.
     */
    function _getTransientContractAddress(bytes32 salt)
        internal
        view
        returns (address)
    {
