...
contract SignatureValidator {
  ...
  function createLoanWithTerms(address callerAddress, LoanRequest memory request, LoanResponse memory response)
    public view
    returns (bool validResponse, address signer, bytes32 requestHash, bytes32 responseHash)
  {
    requestHash = hashRequest(callerAddress, request);
    
    responseHash = hashResponse(response, requestHash);
    
    (validResponse, signer) = validateResponse(response.signer, responseHash, response.signature);
  }
